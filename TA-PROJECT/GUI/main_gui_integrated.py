...
class CryptoGUI(ctk.CTk):
    ...
    def page_metrics(self):
        ...
        # Panel tombol
        control_panel = ctk.CTkFrame(frame, fg_color=COLOR_CARD, corner_radius=12)
        control_panel.grid(row=0, column=0, columnspan=2, sticky="ew", padx=8, pady=8)

        title = ctk.CTkLabel(
            control_panel,
            text="Evaluasi Metrik Baseline dan Optimized",
            font=ctk.CTkFont(size=20, weight="bold"),
            text_color=COLOR_GOLD
        )
        ...
        run_btn = ctk.CTkButton(
            control_panel,
            text="🔄 Hitung Evaluasi Metrik",
            fg_color=COLOR_GOLD,
            text_color="#1B1525",
            hover_color="#E0B900",
            font=ctk.CTkFont(weight="bold"),
            command=self.run_metrics_evaluation
        )
        run_btn.pack(side="right", padx=20, pady=15)

        # Panel baseline
        baseline_panel = ctk.CTkFrame(frame, fg_color=COLOR_CARD, corner_radius=12)
        baseline_panel.grid(row=1, column=0, sticky="nsew", padx=8, pady=8)

        self.baseline_metric_textbox = self.create_metric_table_placeholder(
            baseline_panel,
        ...
        opt_panel.grid(row=1, column=1, sticky="nsew", padx=8, pady=8)

        self.optimized_metric_textbox = self.create_metric_table_placeholder(
            opt_panel,
            "Evaluasi Metrik - Optimized"
        )

        # Panel selisih average
        diff_panel = ctk.CTkFrame(frame, fg_color=COLOR_CARD, corner_radius=12)
        diff_panel.grid(row=2, column=0, columnspan=2, sticky="ew", padx=8, pady=8)

        diff_title = ctk.CTkLabel(
            diff_panel,
            text="Ringkasan Selisih Average",
            font=ctk.CTkFont(size=18, weight="bold"),
            text_color=COLOR_GOLD
        )
        diff_title.pack(anchor="w", padx=20, pady=(15, 5))

        self.average_diff_textbox = ctk.CTkTextbox(
            diff_panel,
            height=160,
            fg_color=COLOR_CARD_2,
            text_color=COLOR_TEXT,
        ...
        self.average_diff_textbox.pack(fill="x", padx=20, pady=(5, 20))

        self.average_diff_textbox.insert(
            "1.0",
            "Metrik              Baseline Avg   Optimized Avg   Selisih     Interpretasi\n"
            "--------------------------------------------------------------------------\n"
            "Menunggu hasil evaluasi metrik."
        )
        self.average_diff_textbox.configure(state="disabled")

        return frame
    
    # ADD COLUMN metrics difference
    def get_average_value(self, metrics, key):
        """
        Mengambil nilai Average dari metrik RGB.
        Jika data berupa dict, ambil bagian 'Average'.
        Jika data berupa angka biasa, langsung gunakan.
        """
        ...
        if isinstance(value, dict):
            return value.get("Average", None)

        return value


    def build_average_difference_table(self, baseline_metrics, optimized_metrics):
        rows = [
            ("Entropy", "Entropy", "higher"),
            ("Corr Horizontal", "Corr Horizontal", "near_zero"),
            ("Corr Vertical", "Corr Vertical", "near_zero"),
            ("Corr Diagonal", "Corr Diagonal", "near_zero"),
            ("NPCR (%)", "NPCR (%)", "higher"),
            ("UACI (%)", "UACI (%)", "target_uaci"),
            ("Waktu Enkripsi", "Enc_Time (s)", "lower"),
            ("Waktu Dekripsi", "Dec_Time (s)", "lower"),
        ...
        for label, key, rule in rows:
            base = self.get_average_value(baseline_metrics, key)
            opt = self.get_average_value(optimized_metrics, key)

            if base is None or opt is None:
                text += f"{label:<19} {'-':>12} {'-':>15} {'-':>10}   Data tidak tersedia\n"
                continue
            ...
            if rule == "boolean":
                ...
                if base_bool == opt_bool:
                    interp = "Sama"
                elif opt_bool and not base_bool:
                    interp = "Optimized lebih baik"
                else:
                    interp = "Optimized lebih buruk"

                text += f"{label:<19} {str(base_bool):>12} {str(opt_bool):>15} {'-':>10}   {interp}\n"
                continue
            ...
            try:
                ...
                if rule == "higher":
                    ...
                    if opt_abs < base_abs:
                        interp = "Lebih mendekati 0"
                    elif opt_abs > base_abs:
                        interp = "Menjauh dari 0"
                    else:
                        interp = "Tetap"

                elif rule == "target_uaci":
                    target = 33.4
                    base_dist = abs(base_val - target)
                    opt_dist = abs(opt_val - target)

                    if opt_dist < base_dist:
                        interp = "Lebih mendekati ideal"
                    elif opt_dist > base_dist:
                        interp = "Menjauh dari ideal"
                    else:
                        interp = "Tetap"
                ...
                text += f"{label:<19} {str(base):>12} {str(opt):>15} {'-':>10}   Gagal dibaca\n"
    ...
    def run_metrics_evaluation(self):
        if not self.process_result or not self.process_result.get("ok"):
            messagebox.showwarning(
                "Proses Belum Ada",
                "Jalankan baseline dan optimized terlebih dahulu pada tab Proses Citra."
            )
            return

        result = self.metric_evaluator.evaluate_process_result(self.process_result)
        self.metrics_result = result

        if not result["ok"]:
            messagebox.showerror("Evaluasi Gagal", result["message"])
            return

        self.update_textbox(
            self.baseline_metric_textbox,
            result["baseline_table"]
        )

        self.update_textbox(
            self.optimized_metric_textbox,
            result["optimized_table"]
        )

        diff_table = self.build_average_difference_table(
            result["baseline"],
            result["optimized"]
        )

        self.update_textbox(
            self.average_diff_textbox,
            diff_table
        )

        messagebox.showinfo(
            "Evaluasi Selesai",
            "Evaluasi metrik baseline, optimized, dan ringkasan selisih average berhasil dihitung."
        )
    ...
    def page_histogram(self):
        ...
        control = ctk.CTkFrame(frame, fg_color=COLOR_CARD, corner_radius=12)
        control.grid(row=0, column=0, columnspan=2, sticky="ew", padx=8, pady=8)
        control.grid_columnconfigure(0, weight=1)

        title = ctk.CTkLabel(
            control,
            text="Histogram RGB Baseline dan Optimized",
            font=ctk.CTkFont(size=20, weight="bold"),
            text_color=COLOR_GOLD
        )
        title.grid(row=0, column=0, sticky="w", padx=20, pady=(16, 5))
        ...
        desc = ctk.CTkLabel(
            control,
            text="Histogram menampilkan distribusi piksel RGB untuk plaintext dan cipher-image.",
            font=ctk.CTkFont(size=14),
            text_color=COLOR_TEXT
        )
        desc.grid(row=1, column=0, sticky="w", padx=20, pady=(0, 16))
        ...
        hist_btn = ctk.CTkButton(
            ...
            hover_color="#E0B900",
            font=ctk.CTkFont(size=15, weight="bold"),
            command=self.run_histogram_generation
        )
        hist_btn.grid(row=0, column=1, rowspan=2, sticky="e", padx=20, pady=16)

        self.hist_baseline_plain_label = self.create_histogram_panel(frame, "Baseline - Plaintext", 1, 0)
        self.hist_baseline_cipher_label = self.create_histogram_panel(frame, "Baseline - Cipher Image", 1, 1)
        self.hist_optimized_plain_label = self.create_histogram_panel(frame, "Optimized - Plaintext", 2, 0)
        self.hist_optimized_cipher_label = self.create_histogram_panel(frame, "Optimized - Cipher Image", 2, 1)
    ...
    def run_histogram_generation(self):
        ...
        result = self.histogram_generator.generate_from_process_result(self.process_result)
        self.histogram_result = result

        if not result["ok"]:
            messagebox.showerror("Histogram Gagal", result["message"])
            return

        paths = result["histogram_paths"]
        self.display_image(self.hist_baseline_plain_label, paths["baseline_plaintext"], max_size=(500, 280))
        self.display_image(self.hist_baseline_cipher_label, paths["baseline_cipher"], max_size=(500, 280))
        self.display_image(self.hist_optimized_plain_label, paths["optimized_plaintext"], max_size=(500, 280))
        self.display_image(self.hist_optimized_cipher_label, paths["optimized_cipher"], max_size=(500, 280))

        messagebox.showinfo(
            "Histogram Selesai",
            "Histogram RGB baseline dan optimized berhasil dibuat."
        )
